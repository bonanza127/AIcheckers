#!/usr/bin/env python3
"""
Train 14d 2-head classifier: CLS (768d) + Stats (14d)

14d = 23d から Negligible (|Cohen's d| < 0.2) を除外
Total input: 768 + 14 = 782d
"""
import argparse
import json
...
DEFAULT_OUT_DIR = Path("/home/techne/aicheckers/models/two_head_14d")

# 14d: Negligible除外後の特徴量
# adj_sim_var, high_sim_ratio, patch_var, norm_var, cls_sim_mean, cls_angle_dispersion,
# knn_sim_var, eigenvalue_ratio, spectral_gap, top3_ratio, degree_centrality,
# clustering_coef, attn_spectral_ratio, center_edge_high_ratio
KEEP_IDX_14D = [1, 2, 3, 5, 9, 12, 14, 15, 16, 17, 18, 19, 24, 32]


def discover_categories(emb_dir: Path):
    """Find categories with CLS and v3 patch stats."""
    cats = []
...
def load_category(name: str, truncate: bool = True):
    """Load CLS and filtered v3 stats (14d)."""
    cls_path = EMBEDDINGS_DIR / f"{name}.npy"
    stats_path = EMBEDDINGS_DIR / f"{name}_patch_stats_v3.npy"

    cls = np.load(cls_path)  # (N, 768)
    stats_v3 = np.load(stats_path)  # (N, 34)

    n_cls = cls.shape[0]
    n_v3 = stats_v3.shape[0]
    n_min = min(n_cls, n_v3)

    if n_cls != n_v3:
        if not truncate:
            raise ValueError(f"{name}: count mismatch cls={n_cls}, v3={n_v3}")
        print(f"[WARN] {name}: count mismatch cls={n_cls}, v3={n_v3} -> truncate {n_min}")
        cls = cls[:n_min]
        stats_v3 = stats_v3[:n_min]

    stats = stats_v3[:, KEEP_IDX_14D]  # (N, 14)
    return cls, stats
...
def main():
    parser = argparse.ArgumentParser(description="Train 14d 2-head classifier")
    ...
    print(f"Stats dim: {ai_stats.shape[1]} (expected 14)")
    ...
    print("Training Head B (Stats 14d)...")
