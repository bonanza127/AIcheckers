#!/usr/bin/env python3
"""
28d_plus_beta: 28d_plusと完全同一条件で検証
- 30 epoch固定
- 最終epochの重みを保存（bestではない）
"""
...
MODEL_DIR = Path('models/two_head_28d_plus_beta')
...
def main():
    print("=" * 60)
    print("Two-Head 28d_plus_beta Training")
    print("Exact same as 28d_plus: 30 epochs, save last weights")
    print("=" * 60)
    ...
    # Save LAST epoch weights (not best)
    MODEL_DIR.mkdir(exist_ok=True, parents=True)
    torch.save(model.state_dict(), MODEL_DIR / "model.pt")

    config = {
        "version": "28d_plus_beta",
        ...
        "notes": "28d_plus exact replica: 30 epochs, last weights saved"
    }
