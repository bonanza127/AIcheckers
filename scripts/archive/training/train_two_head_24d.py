#!/usr/bin/env python3
"""
Train CPU16 + GPU8 2-head classifier: CLS (768d) + Stats (24d)

Stats 구성:
- CPU16d: cpu_stats_v2(13d) + extra(2d) + boundary(1d)
- GPU8d: v3 5d + new 3d (from mid_patches/mid_cls)

Total input: 768 + 24 = 792d
"""
import argparse
import json
...
DEFAULT_OUT_DIR = Path("/home/techne/aicheckers/models/two_head_24d")

# CPU 16d: cpu_stats_v2 13d + extra 2d + boundary 1d
...
# 新規 3d
NEW_FEAT_NAMES = ["local_efficiency", "edge_interior_gap", "cls_sim_center_bias"]


def discover_categories(emb_dir: Path):
    """Find categories with required inputs for CPU16+GPU8."""
    cats = []
...
def compute_new_3d_features(patches, mid_cls, batch_size: int = 32) -> np.ndarray:
    """Compute 3d graph features."""
    ...
    for i in range(0, n_samples, batch_size):
        ...
        for dst, arr in zip(results, [local_eff, edge_gap, center_corr]):
...
def load_category(name: str, truncate: bool = True):
    ...
    gpu_5d = v3[:n_min, GPU_V3_IDX].astype(np.float32)
    gpu_5d = np.nan_to_num(gpu_5d, nan=0.0, posinf=0.0, neginf=0.0)

    new3 = compute_new_3d_features(mid_p[:n_min], mid_c[:n_min])
    gpu = np.concatenate([gpu_5d, new3], axis=1).astype(np.float32)
...
def main():
    parser = argparse.ArgumentParser(description="Train CPU16 + GPU8 2-head classifier (24d)")
    ...
    print(f"Stats dim: {ai_stats.shape[1]} (expected 24)")
    ...
    print("Training Head B (Stats 24d)...")
