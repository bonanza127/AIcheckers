#!/usr/bin/env python3
"""
Train Two-Head classifier using cpu_stats_v2 (18d) + extra_stats (15d)

Output metrics:
- Cohen's d for each feature
- Head B weights
- CLS correlation for each feature
- Inter-feature correlation matrix

構成:
- Head A: CLS (768d)
- Head B: cpu_stats_v2 (18d) + extra_stats_selected (Xd)
"""
import argparse
import json
...
DEFAULT_OUT_DIR = Path("/home/techne/aicheckers/models/two_head_cpu_v2")

# cpu_stats_v2 feature names (18d) - 列2,10はInf含むため除外
CPU_V2_ALL_NAMES = [
    "banding_score",            # 0
...
EXTRA_STATS_NAMES = [
    ...
    "flat_ratio_variance",  # 注: cpu_v2にもあるので除外
]

# extra_statsから選択するインデックス（cpu_v2と重複する6,14は除外）
EXTRA_SELECT_IDX = [0, 1, 2, 3, 4, 5, 7, 8, 9, 10, 11, 12, 13]  # 13d
EXTRA_SELECT_NAMES = [EXTRA_STATS_NAMES[i] for i in EXTRA_SELECT_IDX]


def discover_categories(emb_dir: Path):
    """カテゴリを自動検出"""
    cats = []
    for cpu_path in emb_dir.glob("*_cpu_stats_v2.npy"):
        name = cpu_path.name.replace("_cpu_stats_v2.npy", "")
        cls_path = emb_dir / f"{name}.npy"
        extra_path = emb_dir / f"{name}_extra_stats.npy"

        if not all(p.exists() for p in [cls_path, extra_path]):
            continue
...
def load_category(name: str, emb_dir: Path, max_samples: int = 30000):
    """カテゴリのデータを読み込み"""
    cls_path = emb_dir / f"{name}.npy"
    cpu_v2_path = emb_dir / f"{name}_cpu_stats_v2.npy"
    extra_path = emb_dir / f"{name}_extra_stats.npy"

    cls = np.load(cls_path)
    cpu_v2 = np.load(cpu_v2_path)
    extra = np.load(extra_path)

    # サンプル数を揃える
    n_min = min(cls.shape[0], cpu_v2.shape[0], extra.shape[0], max_samples)

    cls = cls[:n_min].astype(np.float32)
    cpu_v2_selected = cpu_v2[:n_min, CPU_V2_SELECT_IDX].astype(np.float32)
    extra_selected = extra[:n_min, EXTRA_SELECT_IDX].astype(np.float32)

    # NaN/Infを0に置換（念のため）
    cpu_v2_selected = np.nan_to_num(cpu_v2_selected, nan=0.0, posinf=0.0, neginf=0.0)
    extra_selected = np.nan_to_num(extra_selected, nan=0.0, posinf=0.0, neginf=0.0)

    # 統合: cpu_v2_selected (16d) + extra_selected (13d) = 29d
    stats = np.concatenate([cpu_v2_selected, extra_selected], axis=1)
...
def main():
    parser = argparse.ArgumentParser(description="Train Two-Head with cpu_v2 + extra_stats")
    ...
    # 特徴量名（31d = 18 + 13）
    all_feat_names = CPU_V2_NAMES + EXTRA_SELECT_NAMES
    ...
    for i, name in enumerate(all_feat_names):
        ...
        print(f"  {i:2d}. {name:40s} {sign}{d:+.3f}  [{magnitude}]")
    ...
    # ========================================
    print("\n" + "=" * 70)
    print("=== CLS Correlation (per feature) ===")
    print("=" * 70)
    ...
    for i in range(len(all_feat_names)):
        for j in range(i + 1, len(all_feat_names)):
            if abs(corr_matrix[i, j]) > 0.7:
                high_corr_pairs.append((all_feat_names[i], all_feat_names[j], corr_matrix[i, j]))
    ...
    print(f"\nAfter balancing: AI={n_target}, Real={n_target} (1:1)")
    ...
    for i, name in enumerate(all_feat_names):
        ...
        print(f"  {i:2d}. {name:40s} w={weights[i]:+.4f}")
    ...
    torch.save({
        ...
        "cpu_v2_indices": list(range(18)),
        "extra_indices": EXTRA_SELECT_IDX,
    ...
    metrics = {
        ...
        "weights": weight_dict,
        "bias": bias,
        "inter_feature_high_corr": [(n1, n2, float(r)) for n1, n2, r in high_corr_pairs],
        "best_alpha": best,
    }
    ...
    print(f"  - head_a.pt, head_b.pt")
    print(f"  - norm_stats.pt")
    print(f"  - best_alpha.json")
    print(f"  - metrics.json (Cohen's d, weights, correlations)")
    print(f"  - corr_matrix.npy")
