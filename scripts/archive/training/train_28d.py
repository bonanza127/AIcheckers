...
変更点 (29d→28d):
- mid_adj_sim_var を削除 (adj_sim_varと完全重複、計算コスト削減)
- text_area_ratio の inf 対策追加

構成:
- GPU 4d: adj_sim_var[1], patch_var[3], norm_var[5], norm_range[6]
- CPU 24d: CPU16 13d + CPU20 11d (従来通り)
"""
...
MODEL_DIR = Path('models/two_head_28d_v2')
...
def main():
    ...
    print("GPU 4d (no mid_adj_sim_var) + CPU 24d")
    ...
    # Check std
    print("\nChecking std values...")
    for i, std in enumerate(model.gpu_std.cpu().numpy()):
        if std < STD_FLOOR:
            print(f"  GPU[{i}]: std={std:.6f} (will floor)")
    for i, std in enumerate(model.cpu_std.cpu().numpy()):
        if std < STD_FLOOR:
            print(f"  CPU[{i}]: std={std:.6f} (will floor)")
    print("  Done.")
    ...
    config = {
        "version": "28d", "gpu_dim": 4, "cpu_dim": 24,
        "gpu_indices": GPU_4D_IDX, "cpu16_indices": CPU16_13D_IDX, "cpu20_indices": CPU20_11D_IDX,
        "pr_auc": float(early_stopping.best_score), "best_epoch": early_stopping.best_epoch,
        "notes": "Removed mid_adj_sim_var (redundant with adj_sim_var)"
    }
