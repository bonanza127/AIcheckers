#!/usr/bin/env python3
"""
29d Two-Head モデル学習スクリプト

30dから edge_continuity_ratio (std=0.000067, Cohen's d=+0.31) を除外。
正規化爆発の根本原因を解決。

- GPU 5d: adj_sim_var[1], patch_var[3], norm_var[5], norm_range[6], mid_adj_sim_var
- CPU16 13d: banding_score, radial_spectrum_slope, text_area_ratio等（変更なし）
...
# 特徴量インデックス
# GPU: patch_stats_v3から (34dのうち4d選択) + mid_adj_sim_var
# From V3_STAT_NAMES: [1]=adj_sim_var, [3]=patch_var, [5]=norm_var, [6]=norm_range
GPU_4D_IDX = [1, 3, 5, 6]

# CPU16: cpu_stats_v2から (18dのうち13d選択)
CPU16_13D_IDX = [0, 1, 2, 4, 5, 7, 8, 9, 11, 12, 13, 14, 15]

# CPU20: cpu_stats_v3_20dから (20dのうち11d選択)
# REMOVED: index 18 = edge_continuity_ratio (std=0.000067, causes normalization explosion)
CPU20_11D_IDX = [0, 1, 2, 3, 4, 5, 8, 10, 15, 16, 17]  # 18を除外
...
def load_category_data(cat: str) -> tuple:
    ...
    cpu20 = np.load(cpu20_path)[:, CPU20_11D_IDX]  # 11d now
...
class TwoHeadClassifier(nn.Module):
    """Two-Head 29d分類器 (CLS 768d + GPU 5d + CPU 24d = 797d)"""

    def __init__(self, cls_dim=768, gpu_dim=5, cpu_dim=24, hidden_dim=256):
        super().__init__()
        total_dim = cls_dim + gpu_dim + cpu_dim  # 797d
    ...
    def forward(self, cls_feat, gpu_feat, cpu_feat):
        # Z-score正規化 with std floor for safety
        STD_FLOOR = 1e-3
...
def train_model(X_cls, X_gpu, X_cpu, y):
    ...
    print(f"Total: {X_cls.shape[1] + X_gpu.shape[1] + X_cpu.shape[1]}d (+ CLS 768d)")
    ...
    for i, s in enumerate(cpu_std):
        if s < 0.001:
            print(f"  WARNING: cpu_std[{i}] = {s:.6f} (dangerous!)")
...
def main():
    print("=" * 60)
    print("Two-Head 29d Model Training")
    print(f"GPU 5d + CPU 24d (13d + 11d)")
    print(f"Removed: edge_continuity_ratio (std=6.7e-05, d=+0.31)")
