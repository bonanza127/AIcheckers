#!/usr/bin/env python3
"""
Train 22d 2-head classifier: CLS (768d) + Stats (22d)

Removed from 24d:
  - patch_norm_mean (v3_idx 6, w=0.038)
  - low_rank_approx_error (v3_idx 31, w=0.079)

Total input: 768 + 22 = 790d
"""
import argparse
import json
...
DEFAULT_OUT_DIR = Path("/home/techne/aicheckers/models/two_head_22d")

# 22d keep indices (removed v3_idx 6 and 31 from 24d)
KEEP_IDX_22D = [0, 1, 2, 3, 5, 7, 8, 9, 13, 14, 15, 16, 17, 18, 19, 20, 23, 24, 26, 28, 30, 32]
...
def load_category(name: str, truncate: bool = True):
    """Load CLS and filtered v3 stats (22d)."""
    ...
    # Filter to 22d
    stats = stats_v3[:, KEEP_IDX_22D]  # (N, 22)
...
def main():
    parser = argparse.ArgumentParser(description="Train 22d 2-head classifier")
    ...
    print(f"Stats dim: {ai_stats.shape[1]} (expected 22)")
    ...
    print("Training Head B (Stats 22d)...")
