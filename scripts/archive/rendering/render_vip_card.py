...
# Configuration
output_path = "/home/techne/aicheckers/public/campfire-vip-card.png"
...
try:
    ...
    try:
        font_vip = ImageFont.truetype(font_path, 300)
    except OSError:
    ...
    # C. Inner Highlight (Bevel hint) - Optional but adds "expensive" look
    # Move mask up 2 pixels and subtract to find top edge?
    # Keeping it simple: Just paste the clean platinum text
    
    bg.paste(final_text_img, (tx - pad, ty - pad), final_text_img)

    bg.convert("RGB").save(output_path)
    print(f"Luxury VIP Card saved to {output_path}")
